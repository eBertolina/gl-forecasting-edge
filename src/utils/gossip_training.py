...
def get_node_dataset(
    ...
    dataset: Dataset = np.load(
        base_folder / str(simulation_number) / ds_name / f"node_{node_index}.npz", allow_pickle=True
    )
...
def run_simulation(
    ...
    print("Step 2 simulation: check")

    history = simulator.run_training_simulation()

    print("Step 3 simulation: check")
